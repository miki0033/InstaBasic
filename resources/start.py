...
def post_PFP():
    if request.method == "OPTIONS":  # CORS preflight
        ...
        if "file" not in request.files:
            flash("No file part")
            noFileResponse = jsonify(message="There is no file in request")
        ...
        if file and allowed_file(file.filename):
            ...
            response = jsonify(
                message="Image sent succesfully", imageURL="pfp/" + filename
            )
            return response, 200
...
def post_Post():
    if request.method == "OPTIONS":  # CORS preflight
        ...
        if file and allowed_file(file.filename):
            ...
            response = jsonify(
                message="Image sent succesfully", imageURL="posts/" + filename
            )
            return response, 200
